...
ダメージはキャラクター自体のステータスと敵のステータスに左右されます．
元素反応ではないダメージに関係するキャラクター自体のステータスは次の通りです．
...
また，ダメージに関連する敵のステータスは次の通りです．

* 元素耐性による減衰 $R$
* レベルによる減衰（敵の防御力） $L$

これらを用いると，原神における（元素反応ではない）ダメージは次の通り計算できます．

$$
\begin{align*}
\text{Damage w/o Crit.} &= sA(1 + G)RL\\
\text{Damage w/ Crit.} &= sA(1 + D)(1 + G)RL
\end{align*}
$$

一行目は非会心時のダメージで，二行目が会心時のダメージです．
会心率$C$であれば，確率$C$で会心時のダメージとなり，確率$(1-C)$で非会心時のダメージとなります．
従って，ダメージ期待値は次のように計算できます．

$$
\mathbb{E}[\text{Damage}] = sA(1 + CD)(1 + G)RL
$$

もちろん，キャラクターや武器によっては，HPを攻撃力に変換したり，攻撃力の代わりに防御力を参照してダメージを計算したりします．
ただ，原神におけるダメージの多くはこの計算式に従いますので，この計算式のみを説明します．
それでは，それぞれの項目を詳しく説明します．

##### 天賦倍率 $s$

キャラクターの天賦ステータス詳細画面で「～のダメージ XXX%」と書いてある数値です．
天賦レベルによって決まります．

##### 攻撃力 $A$

基本的には攻撃力は基礎攻撃力と倍率（攻撃力%）と加算値で決まり，攻撃力$A$は次のように計算されます．

$$
A = (A_\text{b,c} + A_\text{b,w}) (1 + a_r) + A_\text{add} 
$$

ここで各文字は次のとおりです．

+ キャラクターの基礎攻撃力 $A_\text{b,c}$
+ 武器の基礎攻撃力 $A_\text{b,w}$
+ 累計倍率（攻撃力%） $a_r$
+ 累計加算値 $A_\text{add}$

聖遺物の羽のように加算値（いわゆる実数値）は$A_\text{add}$に足され，攻撃力%はすべて$a_r$へ加算されます．

この中でもレベルによる減衰$L$は敵レベルと自身のレベルのみで決まるため，聖遺物や武器では$L$を変えることはできません．

##### ダメージバフ $G$

ゲーム中では「天賦倍率$s$と攻撃力$A$のみの積$sA$」をダメージと呼称しています．
この値は実際に敵に与えられる（画面に表示される）ダメージとは異なりますが，たとえばポケモンでいえば「技の威力」がこれに当たると考えるとわかりやすいかと思います．
この技の威力に対して，ダメージのバフや会心，相手による減衰の計算をすることで最終的なダメージが計算されます．

ダメージのバフの最も典型的な例がダメージバフと呼ばれているもので，たとえば聖遺物の杯の「～元素ダメージ」とか「物理ダメージ」などの数値がこれに当たります．
たとえば，「ダメージ+30%」とは「ダメージ値$sA$に30%のボーナスを与える」という意味なので，ダメージ値が「$1.3 \times sA$」となります．
一般的にはダメージバフの累計値を$G$とすると，ダメージバフを考慮したダメージ値は$sA(1+G)$となります．


##### 会心率$C$・会心ダメージ$D$

ダメージ計算式の中で唯一確率的な要素になっているのが会心系です．
いわゆるクリティカルとか急所とか，そういう概念です．
会心率が$C$とは，確率$C$で会心が発生し，確率$(1-C)$で非会心であるという意味です．
非会心時のダメージを$X$とすると，会心時のダメージは$X(1+D)$になります．
つまり，キャラクターの会心ダメージが100%とは，会心によってダメージが2倍になること意味します．


##### 耐性による減衰 $R$

敵は各元素や物理に対して耐性を持っています．
ヒルチャールなどの一般の敵は，すべての元素に対して一律に10%の耐性を持っています．
一方で，遺跡守衛系は物理に対して高い耐性を持っていたり，炎のトリックフラワーは炎元素への耐性が高くなっていたりします．

耐性値が$x$のとき，減衰量$R$は少し複雑な計算になります．
...
たとえば，元々の耐性が10%で，翠緑4セット効果で30%耐性が下がった場合は$x = 0.1 - 0.3 = -0.2$となります．
このとき，耐性による減衰値は$R = 1 - \frac{-0.2}{2} = 1.1$となり，1.1倍になります（減衰どころか増加します）．


##### レベルによる減衰 $L$

実は相手のレベルと自分のレベルによってダメージは変わります．
この減衰量は次のように計算されます．

$$
L^{-1}  = 1 + \frac{1 + l_e}{1 + l_c} (1 - b_r)
$$

上の式では綺麗に記述するために，左辺が逆数$L^{-1}$になっていることに注意してください．
ここで，$l_e$と$l_c$はそれぞれ敵と自身のレベルを100で割った値（90レベルなら0.9）です．
また，$b_r$は敵の防御力を低下させる効果です（ちなみにこれとは別に「防御力を無視する効果」も存在します）．
もし，敵と自身のレベルが等しく（$l_e = l_c$），防御力を低下させる効果がない（$b_r = 0$）のであれば$L=1/2 = 0.5$となりダメージは半減します．
敵のレベルは高いほどに敵の防御力は高く，ダメージは低下します．
